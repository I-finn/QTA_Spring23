...
#corpSum22$Flesch.Kincaid <- textstat_readability(corpSum22$Text)['Flesch']
corpSum22$fk <- textstat_readability(corpSum22$Text)
...
nameString <- "Luke Harding|Julian Borger|Jennifer Rankin"
corpSum22 %>%
  filter(grepl(pattern=nameString, x=byline)) 

corpSum22 %>%
  filter(grepl(pattern=nameString, x=byline)) %>%
  group_by(grp = str_extract(byline, nameString)) %>%
  summarise(av = mean(fk$Flesch)) %>%
...
corpSum23$fk <- textstat_readability(corpSum23$Text)
corpSum23 %>%
  filter(grepl(pattern=nameString, x=byline)) %>%
  group_by(grp = str_extract(byline, nameString)) %>%
  summarise(av = mean(fk$Flesch)) %>%
...
# look at list of collocations and pick a cut off point
...
toks22<- tokens_remove(toks22, pattern=extra_stopwords)
toks23<- tokens_remove(toks23, pattern=extra_stopwords)
dfm22 <- dfm(toks22)
topfeatures(dfm22)
dfm23 <- dfm(toks23)
topfeatures(dfm23)
...
#consider revising tokens to replace leopard with leopard tank
